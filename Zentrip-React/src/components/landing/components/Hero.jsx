export default function Hero({ onRegister }) {
  return (
    <section className="min-h-screen bg-slate-50 flex items-center pt-16 px-16 gap-16 relative overflow-hidden">
      {/* blobs */}
      ...
      <div className="flex-1 relative z-10 max-w-xl">
        ...
        <h1 className="text-6xl font-extrabold text-blue-900 leading-tight mb-6" style={{ fontFamily: "'Georgia', serif" }}>
          Viaja sin caos,{" "}
          <span className="italic text-orange-500">disfruta</span>{" "}
          cada momento.
        </h1>
        ...
        <div className="flex items-center gap-4 mb-10">
          <button
            onClick={onRegister}
            className="px-8 py-4 rounded-2xl border-none text-base font-extrabold text-white cursor-pointer transition-all"
            style={{ background: "linear-gradient(135deg, #FF6B2C, #FF8C00)", boxShadow: "0 8px 24px rgba(255,107,44,.35)" }}>
            Empieza gratis →
          </button>
          <button className="px-8 py-4 rounded-2xl border-2 border-slate-200 text-base font-bold text-blue-900 bg-white hover:bg-slate-50 cursor-pointer transition-colors">
            Ver cómo funciona
          </button>
        </div>
      ...
      <div className="flex-1 relative z-10 flex items-center justify-center">
        <div className="relative">
          ...
          {/* Float 1 */}
          <div className="absolute -top-5 -right-8 bg-white rounded-2xl px-4 py-3 flex items-center gap-3"
